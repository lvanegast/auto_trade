...
9. **SportsMatcher Semántico Cross-Exchange (`backend/src/utils/sports_matcher.py`)**: Inspirado en `dr-manhattan` (`cross_exchange/matcher.py`) y `DRADIS`. Normalizador canónico con diccionario de más de 120 entidades (Fútbol EPL, La Liga, Serie A, UCL, MLS; Tenis ATP/WTA; Esports; NFL/NBA). Realiza ordenamiento alfabético determinista de los contrincantes (`team_a < team_b`) para que eventos con títulos invertidos entre Limitless y Polymarket US resuelvan al mismo ID canónico exacto. Homologa outcomes ("Draw", "Tie", "Draw (Reg. Time)" -> "draw"; "3+ total goals", "Over 2.5 goals" -> "over-2.5-goals"). Integrado en `event_id.py`, `multi_platform_feeder.py` y `cross_platform_tracker.py`.
10. **Modelo Atómico de 9 Estados para Fútbol en Dual Simplex LP (`backend/src/strategy/combinatorial_arb.py`)**: Partición matemática exacta de 9 estados mutuamente excluyentes y colectivamente exhaustivos combinando Ganador 3-way (`TEAM_A_WIN`, `TEAM_B_WIN`, `DRAW`), Goles (`OVER_2_5`, `UNDER_2_5`) y Ambos Marcan (`BTTS`). Resuelve la canasta óptima de cobertura $\min c^T x$ sujeto a $A x \ge \mathbf{1}, x \ge 0$, garantizando payout de exactamente $1.0000$ en cualquier resultado del partido si el costo total $c^* < 1.00$.
11. **Hardening de Ejecución Maker-Taker y Scratch Unwind en Worker 6 (`supervisor.py` & `maker_taker_coordinator.py`)**: Bucle rápido `_maker_fast_sync` a 500ms para detección instantánea de fills parciales. Si una pierna se ejecuta, envía orden FOK inmediata para cerrar la pata opuesta a precio de mercado si el costo $\le 0.985$. Si las condiciones cambiaron desfavorablemente (>0.985), ejecuta Scratch Unwind (`SELL FOK`) al bid de mercado para evitar quedarse expuesto al settlement del contrato.
