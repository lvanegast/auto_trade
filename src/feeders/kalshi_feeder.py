...
class KalshiFeeder(BaseFeeder):
    ...
    async def start(self):
        ...
        if not self.api_key_id or not self.private_key_path:
            print(f"[Feeder Kalshi] Credenciales no configuradas. Iniciando Feed Simulado para {self.symbol}...")
            self.task = asyncio.create_task(self._run_mock_stream())
        else:
            print(f"[Feeder Kalshi] Iniciando conexión real con Kalshi WebSocket ({self.environment}) para {self.symbol}...")
    ...
    async def _run_real_stream(self):
        ...
        while self.running:
            try:
                ...
                print(f"[Feeder Kalshi] Error en conexión WebSocket: {e}. Reintentando en 5 segundos...")
